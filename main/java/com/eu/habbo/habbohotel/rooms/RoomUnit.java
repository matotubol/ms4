...
public class RoomUnit {
    ...
    public boolean cycle(Room room) {
        try {
            if (handleRider(room)) return this.statusUpdate;

            Habbo habboT = room.getHabbo(this);
            if (isNotWalkingAndNotKicked(habboT)) {
            ...
            Deque<RoomTile> peekPath = retrievePeekPath(room);
            RoomTile next = retrieveNextTile(peekPath);

            if (next == null) return true;
            if (!handleUserMovementEvents(room, next)) return true;

            HabboItem item = handleItemInteractions(room, next);

            double zHeight = calculateZHeight(room, next, habboT, item);

            handleRoomUnitMovements(room, next, zHeight, habboT, item);
    ...
    private boolean isNotWalkingAndNotKicked(Habbo habboT) {
        return !this.isWalking() && !this.isKicked() && this.status.remove(RoomUnitStatus.MOVE) == null && habboT != null;
    }
    ...
    private Deque<RoomTile> retrievePeekPath(Room room) {
        if (this.path == null || this.path.isEmpty()) return new LinkedList<>();
    ...
    private RoomTile retrieveNextTile(Deque<RoomTile> peekPath) {
        RoomTile next = this.path.poll();
        if (this.fastWalk && this.path.size() > 1) {
            next = this.path.poll();
        }
        return next;
    }

    private boolean handleUserMovementEvents(Room room, RoomTile next) {
    ...
    private HabboItem handleItemInteractions(Room room, RoomTile next) {
        ...
        if (habboItem != null) {
            try {
                ...
                log.error("Error in onWalkOff for item: " + habboItem.getId(), e);
            }
        }
        ...
        if (item != null) {
            try {
                ...
                log.error("Error in onWalk for item: " + item.getId(), e);
            }
        }
    ...
    private double calculateZHeight(Room room, RoomTile next, Habbo habboT, HabboItem item) {
        double zHeight = 0.0D;
    ...
    private void handleRoomUnitMovements(Room room, RoomTile next, double zHeight, Habbo habboT, HabboItem item) {
        this.tilesWalked++;
        ...
        if (this.canLeaveRoomByDoor && isAtDoor && publicRoomKicks && !invalidated) {
            Emulator.getThreading().run(new RoomUnitKick(habboT, room, false), 500);
        }

        if (habboT != null) {
            if (habboT.getHabboInfo().getRiding() != null) {
                RoomUnit ridingUnit = habboT.getHabboInfo().getRiding().getRoomUnit();
